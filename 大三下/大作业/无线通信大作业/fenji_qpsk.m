%================= STEP 0: 初始化环境 ===================
clc; clear; close all;
% 清除命令窗口、工作区变量和关闭所有图形窗口，确保仿真开始时环境干净

%================= STEP 1: 设置仿真参数 ===================
N = 1e5;                          % 每种设置下的比特数（即每次仿真的总比特数）
SNR_dB = 0:2:20;                  % 定义信噪比范围：从0到20 dB，步长为2 dB
mod_type = 'QPSK';                % 当前仅使用 QPSK 调制方式
M = 4;                            % QPSK对应的调制阶数（4个符号点）
k = log2(M);                      % 每个符号携带的比特数（log2(4)=2）
Nr_antennas = 1:5;                % 接收天线数量测试集（L = 1 到 5）
combine_methods = {'SC', 'MRC', 'EGC'}; % 分集合并方式：选择合并、最大比合并、等增益合并

% 预分配 BER 存储空间（用于保存不同合并方式和不同天线数的误码率结果）
...
% 使用QAM调制将比特转换为复数符号（QPSK），单位平均功率归一化
...
for idx = 1:length(L_vals)
    ...
    % 创建新图形窗口，标题中显示当前天线数
    figure('Name', ['QPSK 合并方式对比 - L = ' num2str(L)], 'Color','w');
    ...
    % 添加标题和坐标轴标签
    title(['QSNR (dB)', 'FontSize', 12);
    ylabel('PSK 合并方式对比（L = ' num2str(L) '）'], 'FontSize', 14, 'FontWeight', 'bold');
    xlabel('误码率（BER）', 'FontSize', 12);
    legend('SC','EGC','MRC','Location','southwest'); % 添加图例
    xlim([min(SNR_dB) max(SNR_dB)]); % 设置横轴范围
    ylim([1e-4 1]); % 设置纵轴范围（BER）
    
    % 保存图片为 PNG 文件
    saveas(gcf, ['QPSK_L' num2str(L) '_Compare.png']);
end
...
writetable(T, 'QPSK_合并方式性能统计.xlsx', 'WriteRowNames', true);
% 将统计表格写入名为 "QPSK_合并方式性能统计.xlsx" 的 Excel 文件中